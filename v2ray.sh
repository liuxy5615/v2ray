...
if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/masteen/v2ray ]]; then
	...
	. /etc/v2ray/masteen/v2ray/tools/v1xx_to_v3xx.sh
...
if [[ $mark != "v3" ]]; then
	. /etc/v2ray/masteen/v2ray/tools/v3.sh
fi
...
. /etc/v2ray/masteen/v2ray/src/init.sh
systemd=true
# _test=true
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}123456$none)"): " new_sspass
		[ -z "$new_sspass" ] && new_sspass="123456"
		case $new_sspass in
		*[/$]*)
			echo
...
path_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认: [${cyan}masteen$none]):")" new_path
		[[ -z $new_path ]] && new_path="masteen"
...
update_v2ray.sh() {
	if [[ $_test ]]; then
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://github.com/liuxy5615/v2ray/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	else
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://github.com/liuxy5615/v2ray/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	fi
...
_help() {
	echo
	echo "........... V2Ray 管理脚本帮助信息 by masteen .........."
	echo -e "
