...
socks_user_config() {
	echo
	while :; do
		read -p "$(echo -e "请输入$yellow用户名$none...(默认用户名: ${cyan}masteen$none)"): " new_socks_username
		[ -z "$new_socks_username" ] && new_socks_username="masteen"
...
socks_pass_config() {
	echo
	while :; do
		read -p "$(echo -e "请输入$yellow密码$none...(默认密码: ${cyan}123456.com$none)"): " new_socks_userpass
		[ -z "$new_socks_userpass" ] && new_socks_userpass="123456"
